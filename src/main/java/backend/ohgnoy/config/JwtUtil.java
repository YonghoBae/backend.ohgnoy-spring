...
public class JwtUtil {
    ...
    public String generateToken(User user) {
        ...
        return Jwts.builder()
                // .claims() : 위에서 정의한 추가 정보를 Payload에 담는다 (기존 addClaims)
                .claims(claims)
                // .subject() : 토큰의 '주체'를 설정, 주로 사용자를 식별할 수 있는 고유한 값을 사용 (기존 setSubject)
                .subject(user.getEmail())
                // .issuedAt() : 토큰이 발급된 시간을 설정 (기존 setIssuedAt)
                .issuedAt(now)
                // .expiration() : 토큰의 만료 시간을 설정, 이 시간이 지나면 토큰은 유효하지 않게 됨 (기존 setExpiration)
                .expiration(expiration)
                // .signWith() : 토큰에 서명을 추가. getSigningKey()로 가져온 비밀 키를 사용
    ...
    private Claims getClaimsFromToken(String token) {
        //Jwts.parser()를 사용하여 JWT 파서를 생성
        return Jwts.parser()
                // .verifyWith() : 토큰을 검증할 때 사용할 비밀 키를 설정. 발급할 때 사용한 키와 동일 (기존 setSigningKey)
                .verifyWith(getSigninKey())
                // .build() : 파서를 빌드
                .build()
                // .parseSignedClaims(token): 토큰 문자열을 파싱하여 서명을 검증하고, Claims를 추출 (기존 parseClaimsJws)
                // 만약 서명이 다르거나 토큰이 변조되었다면 여기서 예외가 발생
                .parseSignedClaims(token)
                // .getPayload(): 파싱된 JWT의 Payload(Body) 부분, 즉 Claims 객체를 반환 (기존 getBody)
                .getPayload();
    }
    ...
    public String getEmailFromToken(String token) {
        //getClaimsFromToken() 메소드를 호출하여 전체 Claims를 얻어온 뒤,
